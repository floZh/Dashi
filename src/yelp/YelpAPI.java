...
public class YelpAPI {
	...
	private static final String CLIENT_ID = "9-5C1V9YASEz-tjgb1CBdA";
	private static final String CLIENT_SECRET = "LPvwUy3pOt7ltDzZsbMNG3PAzIjczPn4J7s1SMDfZl8GkgwaF10leCSxFmMA83XZ";
	...
	private JSONObject obtainAccessToken() {
		try {
			...
			HttpURLConnection connection = (HttpURLConnection) new URL(TOKEN_HOST).openConnection(); //建立http链接

			connection.setDoOutput(true);
			connection.setRequestMethod("POST");

			connection.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");  //设置format，发请求
			...
			while ((inputLine = in.readLine()) != null) {    //读请求结果
				response.append(inputLine);
			}
			in.close();
			return new JSONObject(response.toString());   //返回json格式的结果
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
